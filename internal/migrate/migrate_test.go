...
func TestAgentConfigFromAgentCoversPersistedFields(t *testing.T) {
	...
	src := config.Agent{
		...
		ScaleCheck:             "echo 3",
		ScaleCheckQuery:        config.WorkSelector{Type: "task", Unassigned: true, Metadata: map[string]string{"gc.routed_to": "worker"}},
		DrainTimeout:           "10m",
		OnBoot:                 "echo boot",
		OnDeath:                "echo death",
		Namepool:               "names.txt",
		...
		WorkSelector:           config.WorkSelector{Type: "task", Unassigned: true, Metadata: map[string]string{"gc.routed_to": "worker"}},
		IdleTimeout:            "15m",
		MaxSessionAge:          "5h",
