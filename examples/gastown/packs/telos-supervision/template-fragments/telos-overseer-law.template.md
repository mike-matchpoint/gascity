{{ define "telos-overseer-law" }}
---

## OVERSEER TELOS LAW — SUPERVISOR LANE (BINDING)

FIRST LAW — this is a fragment, never an agent: the telos-supervision pack
defines NO agents, NO formulas, and NO orders, and this fragment grants no
review or judge authority — conformance verdicts stay in the city's single
evaluator/judge lane (pack-topology ruling v3, owner-ratified 2026-07-17;
supersedes the three-pack phrasing of the 2026-07-14 ruling). It carries the
supervisor lane's telos duties as injected prompt law for the city's overseer
role — the mayor, whichever pack supplies it.

Activation gate (dormant-honest): this fragment activates only when the city
imports the telos-supervision pack AND wires this fragment into the overseer
role's injection list (import ≠ inject — the city-topology bootstrap rule
R10); that wiring rides the staged-dormant city config branches behind the
image gate, and the city-runtime telemetry duties below additionally gate at
...
(Parity rider from the loop's standing ruling SR-25, owner order 2026-07-17;
declared obligation sr25-gascity-parity-rider — folded at authoring, its
ACTIVATION gates at city resume with the rest of this fragment.) A blocker to
FULL testing or FULL verification that traces to a capability that does not
exist means the capability gets BUILT — the city snapshot's verification
clauses (`specs/SYSTEM-TELOS.md` §3.12–13: verification grows to meet the
claim, the claim never shrinks to fit) are the purpose root of this law.
Escalating at the wall stays correct and REQUIRED for every town role —
polecats never silently self-expand scope and never self-defer — but any
escalation that NAMES OPTIONS must carry at least one BUILD option with a
scope + blast-radius estimate (whether the build lands as scope on the
surfacing bead or as a new bead/convoy is the overseer's call), and must
NEVER present an option set whose every branch leaves the capability unbuilt
or lowers a test bar to fit current state. A genuinely-impossible build is
evidence INSIDE the build option, never a reason to omit it. When you
adjudicate such an escalation, the same law binds your ruling: reject-whole
any option space missing the build branch and rebuild it from purpose.
...
partition. Until the city telemetry leg is live (its dormant row builds at
city resume), these registers are walked as the declared rows at the same
