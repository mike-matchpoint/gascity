...
func buildDoctorChecks(cityPath string, cfg *config.City, cfgErr error, opts buildDoctorChecksOpts) []doctor.Check {
	...
	if cfgErr == nil && cfg != nil {
		...
		register(newRoutedWorkDemandContractCheck(cfg, cityPath, storeFactory))
	...
	register(doctor.NewDoltContentionCheck(cityPath))
	register(doctor.NewRuntimeWriteCheck(cityPath))
